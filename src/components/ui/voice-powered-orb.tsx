...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const externalAudioLevelRef = useRef(externalAudioLevel);

  // Keep external audio level ref in sync
  useEffect(() => {
    externalAudioLevelRef.current = externalAudioLevel;
  }, [externalAudioLevel]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        // Combine mic voice level with external audio level (AI speaking)
        let combinedLevel = 0;

        if (enableVoiceControl && isMicrophoneInitialized) {
          voiceLevel = analyzeAudio();
          combinedLevel = Math.max(voiceLevel, externalAudioLevelRef.current);

          if (onVoiceDetected) {
            onVoiceDetected(voiceLevel > 0.1);
          }
        } else {
          // Even without mic, react to external audio (AI speaking)
          combinedLevel = externalAudioLevelRef.current;
          if (onVoiceDetected) {
            onVoiceDetected(false);
          }
        }

        // Map combined level to rotation speed
        const voiceRotationSpeed = baseRotationSpeed + (combinedLevel * maxRotationSpeed * 2.0);

        if (combinedLevel > 0.05) {
          currentRot += dt * voiceRotationSpeed;
        }

        program.uniforms.hover.value = Math.min(combinedLevel * 2.0, 1.0);
        program.uniforms.hoverIntensity.value = Math.min(combinedLevel * maxHoverIntensity * 0.8, maxHoverIntensity);
